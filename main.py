...
# Fitur Baru: System Health Monitoring (Deteksi Delay Server)
...
# JADWAL CRON ANDA (Dalam Menit). 
# Ubah angka ini jika Anda mengubah jadwal di main.yml!
# Jika di main.yml adalah cron: '50 * * * *', maka isi 50.
...
def generate_signals_and_predict(df, news_score):
    ...
    for i in range(len(df)):
        ...
        if row['StochRSI'] < 0.2: score += 1 
        elif row['StochRSI'] > 0.8: score -= 1 
        if row['Close'] > row['VWAP_24']: score += 1
        else: score -= 1

        trend_strength = 1.5 if row['ADX'] > 25 else 0.5 
...
def evaluate_previous_prediction(df):
    ...
    if 'BELI' in prev_signal:
        ...
        if abs(persentase) <= 0.3: return f"TEPAT ⚪ (Pasar memang sepi: {persentase:.2f}%)"
        elif persentase > 0.3: return f"TERLEWAT ⚠️ (Sinyal HOLD, tapi harga Naik {persentase:.2f}%)"
        else: return f"TERLEWAT ⚠️ (Sinyal HOLD, tapi harga Turun {persentase:.2f}%)"
...
def plot_professional_analysis(df, filename="chart.png"):
    ...
    ax1.scatter(next_time, next_price, color='red', s=250, marker='*', zorder=10, label=f'Titik Prediksi 1 Jam: {format_rupiah(next_price)}')
...
def main():
    ...
    if len(df) > 2:
        ...
        # STOP TIMER & HITUNG LAMA PROSES
        end_time = time.time()
        waktu_eksekusi_script = end_time - start_time
        
        # HITUNG DELAY SERVER GITHUB
        sekarang_wita = datetime.now(pytz.timezone('Asia/Makassar'))
        menit_sekarang = sekarang_wita.minute
        
        # Logika menghitung selisih menit keterlambatan
        if menit_sekarang >= EXPECTED_CRON_MINUTE:
            delay_menit = menit_sekarang - EXPECTED_CRON_MINUTE
        else:
            delay_menit = (menit_sekarang + 60) - EXPECTED_CRON_MINUTE
            
        if delay_menit <= 5:
            info_server = "⚡ Tepat Waktu (Lancar)"
        else:
            info_server = f"🐢 Terkena Delay Antrean GitHub ({delay_menit} menit)"
        
        # Format Laporan
        pesan = f"💎 *LAPORAN ANALISIS BITCOIN PRO MAX* 💎\n"
        pesan += f"_{latest.name.strftime('%d %B %Y | Data Jam %H:%M WITA')}_\n\n"
        
        # INI TAMBAHAN INFORMASI SYSTEM HEALTH
        pesan += f"⚙️ *System Health Monitoring:*\n"
        pesan += f"├ GitHub: {info_server}\n"
        pesan += f"└ Python: {waktu_eksekusi_script:.1f} detik (Kecepatan proses)\n\n"
        ...
        if 'BELI' in latest['Recommendation']:
            ...
            pesan += f"💡 _Saran: Pasang Stop Loss di angka {format_rupiah(sl)} dan Take Profit di {format_rupiah(tp)}._\n"
