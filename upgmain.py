...
def engineer_features(df, fng_dict):
    ...
    features_cols = [
        ...
        "Volume_Spike", "Dist_to_Res", "Dist_to_Sup", "Swing_High", "Swing_Low"
    ]
    return df, features_cols

def train_honest_model(df, features, target_col, shift_len, model_name, current_regime):
    model_file = f"ai_model_{model_name}_{current_regime}.pkl"
    need_training = True
    ...
    if need_training:
        logging.info(f"Training Model {model_name} for Regime: {current_regime}...")
        ...
        if len(regime_df) < 100:
            logging.warning(f"Data rezim {current_regime} tidak cukup ({len(regime_df)} baris). Menggunakan semua data (Fallback).")
            regime_df = train_df
            
        X_train = regime_df[features].values
        y_train = regime_df[target_col].values
        ...
        xgb_base = XGBClassifier(
            ...
            eval_metric='logloss', scale_pos_weight=ratio_weight
        )
        ...
        if file_exists:
            ...
            for idx, row in history[mask_1h].iterrows():
                ...
                if t_target in df.index:
                    ...
                    if actual_end_price_idr > batas_naik_1h: arah_asli = "Significant Uptrend"
                    elif actual_end_price_idr > past_idr_price: arah_asli = "Slight Uptrend"
                    elif actual_end_price_idr < batas_turun_1h: arah_asli = "Significant Downtrend"
                    else: arah_asli = "Slight Downtrend"
                    
                    if (past_prob > 0.5 and actual_end_price_idr > batas_naik_1h) or (past_prob <= 0.5 and actual_end_price_idr <= batas_naik_1h):
            ...
            for idx, row in history[mask_6h].iterrows():
                ...
                if t_target in df.index:
                    ...
                    batas_naik_6h = past_idr_price * 1.015 
                    batas_turun_6h = past_idr_price * 0.985
                    
                    if actual_end_price_idr > batas_naik_6h: arah_asli = "Significant Uptrend"
                    elif actual_end_price_idr > past_idr_price: arah_asli = "Slight Uptrend"
                    elif actual_end_price_idr < batas_turun_6h: arah_asli = "Significant Downtrend"
                    else: arah_asli = "Slight Downtrend"
                    
                    if (past_prob > 0.5 and actual_end_price_idr > batas_naik_6h) or (past_prob <= 0.5 and actual_end_price_idr <= batas_naik_6h):
                        hasil = f"SUCCESS ({arah_asli})"
                    else:
                        hasil = f"FAILED ({arah_asli})"
    ...
    try:
        ...
        if not is_duplicate:
            with open(HISTORY_FILE, mode='a', newline='') as file:
                ...
                writer.writerow([
                    t_curr_str, kurs_idr, global_usd, indodax_live_idr, 
                    t_1h_str, prob_1h, '', '', '', 
                    t_6h_str, prob_6h, '', '', ''
                ])
    except:
    ...
    try:
        ...
        web_data = {
            ...
            "prices": {
                ...
                "spread_premium": 0 # Dihapus dari logika utama, dibiarkan 0 untuk dashboard lawas
            },
            "current_prediction": {
                "prob_1h": prob_1h,
                "prob_6h": prob_6h,
...
def get_dynamic_thresholds(df, base_1h=0.08, base_6h=0.15):
    vol = df["Volatility"].iloc[-1]
    vol_mean = df["Volatility"].rolling(100).mean().iloc[-1]
...
def position_sizing_kelly(prob_1h, prob_6h, atr, risk_multiplier, threshold_1h, threshold_6h):
    threshold_avg = (threshold_1h + threshold_6h) / 2
...
def plot_professional_analysis(df, prob_1h, prob_6h, atr, base_filename="chart_main"):
    ...
    plt.title('INDODAX LOCAL DUAL ENGINE (80 HOURS) - EVALUATED IN IDR', fontsize=16, fontweight='bold')
...
def plot_zoomed_analysis(df, prob_1h, prob_6h, atr, base_filename="chart_zoom"):
    ...
    ax1.annotate(f"IDR {last_price:,.0f}", (last_time, last_price), xytext=(0, -25), textcoords='offset points', ha='center', bbox=dict(boxstyle="round", fc="black", alpha=0.8), color="white")
    ax1.set_title('ZOOM BOLLINGER (INDODAX IDR)', fontsize=16, fontweight='bold')
...
def send_telegram_messages(pesan_utama, chart_paths, pesan_diag=""):
    if not TELEGRAM_BOT_TOKEN: 
        ...
        if pesan_diag.strip() != "": 
            print("\n[DIAGNOSTIK]:\n" + pesan_diag)
        print(f"\n📸 Chart yang di-generate: {', '.join(chart_paths)}")
        print("="*50 + "\n")
        return
...
def main():
    ...
    try:
        ...
        df, kurs_idr, global_usd, indodax_live_idr = fetch_data_with_retry(days_back=60)
        ...
        try: 
            prob_1h = train_honest_model(df_closed, features, "Target_1H", shift_len=1, model_name="1H", current_regime=current_regime)
        except Exception as e: 
            diagnostics["ai_1h"] = f"Error: {e}"
            prob_1h = 0.5
            
        try: 
            prob_6h = train_honest_model(df_closed, features, "Target_6H", shift_len=6, model_name="6H", current_regime=current_regime)
        except Exception as e: 
        ...
        # LOGIKA TRADING DISEMPURNAKAN (Langsung ke Poin Eksekusi)
        if prob_1h > threshold_1h and prob_6h > threshold_6h:
            if status_berita == "HIGHLY NEGATIVE":
                saran_tindakan = "HOLD (Tahan Dulu)"
                alasan_saran = "AI mendeteksi Uptrend, namun berita pasar sedang sangat buruk/FUD."
            elif sell_wall > (buy_wall * 3):
                saran_tindakan = "HOLD (Halangan Sell Wall)"
                alasan_saran = "AI mendeteksi Uptrend, tapi ada tembok antrean JUAL raksasa di Indodax."
            elif rsi_sekarang < 35:
                saran_tindakan = "WAIT (Tunggu Reversal)"
                alasan_saran = "Harga sedang diskon tajam (Oversold), tunggu konfirmasi pantulan."
            else:
                saran_tindakan = "STRONG BUY"
                alasan_saran = "Seluruh indikator & AI menyarankan NAIK. Momentum sangat bagus!"
        elif prob_1h <= threshold_1h and prob_6h <= threshold_6h:
            if rsi_sekarang > 70:
                saran_tindakan = "TAKE PROFIT (Jual Sebagian)"
                alasan_saran = "AI memprediksi turun dan harga saat ini sudah kemahalan (Overbought)."
            else:
                saran_tindakan = "STRONG SELL"
                alasan_saran = "AI mengkonfirmasi terjadinya tren TURUN yang cukup valid."
        else:
            saran_tindakan = "NEUTRAL / HOLD"
            alasan_saran = "Sinyal 1 Jam & 6 Jam berlawanan arah. Pasar sedang membingungkan."
            
        if fng_val >= 75: fng_str = f"{int(fng_val)} (Extreme Greed)"
        elif fng_val >= 55: fng_str = f"{int(fng_val)} (Greed)"
        elif fng_val <= 25: fng_str = f"{int(fng_val)} (Extreme Fear)"
        elif fng_val <= 45: fng_str = f"{int(fng_val)} (Fear)"
        else: fng_str = f"{int(fng_val)} (Neutral)"
        
        if buy_wall > sell_wall:
            status_orderbook = f"Banyak Pembeli ({(buy_wall/sell_wall if sell_wall>0 else 1):.1f}x dari Penjual)"
        else:
            status_orderbook = f"Banyak Penjual ({(sell_wall/buy_wall if buy_wall>0 else 1):.1f}x dari Pembeli)"
        ...
        if not os.path.exists(ALERT_FILE):
            kirim_telegram = True
            alasan_kirim = "Inisialisasi Sistem Baru"
            is_routine_update = True
        elif ada_error_sistem:
            kirim_telegram = True
            alasan_kirim = "Gangguan Sistem / API"
        else:
            try:
                with open(ALERT_FILE, 'r') as f:
                    last_state = json.load(f)
                last_price_saved = last_state.get('price', indodax_live_idr)
                if last_price_saved > 0:
                    raw_diff_pct = ((indodax_live_idr - last_price_saved) / last_price_saved) * 100
                else:
                    raw_diff_pct = 0.0
                if raw_diff_pct > 0: teks_perubahan = f"Naik +{raw_diff_pct:.2f}%"
                elif raw_diff_pct < 0: teks_perubahan = f"Turun {raw_diff_pct:.2f}%"
                ...
                if price_diff >= 0.6:
                    kirim_telegram = True
                    alasan_kirim = f"Pergerakan Harga Cepat"
                elif saran_tindakan in ["STRONG BUY", "STRONG SELL", "WAIT (Tunggu Reversal)", "TAKE PROFIT (Jual Sebagian)"] and saran_tindakan != last_state.get('saran', ''):
                    kirim_telegram = True
                    alasan_kirim = f"PERUBAHAN SINYAL PENTING"
                elif saran_tindakan != last_state.get('saran', ''):
                    if selisih_waktu < 3600: kirim_telegram = False 
                    else:
                        kirim_telegram = True
                        alasan_kirim = f"Update Rekomendasi"
                elif 5 <= menit_sekarang < 20 and last_hourly_report != waktu_sekarang_str:
                    kirim_telegram = True
                    alasan_kirim = f"Laporan Rutin AI"
                    is_routine_update = True
                else:
                    kirim_telegram = False
            except Exception:
                kirim_telegram = True
                alasan_kirim = "Reset Status Memori"
                is_routine_update = True
        ...
        if kirim_telegram:
            ...
            # FORMAT TELEGRAM BARU (QUICK-READ & ACTION-FIRST)
            if "BUY" in saran_tindakan: icon_aksi = "🟢"
            elif "SELL" in saran_tindakan or "PROFIT" in saran_tindakan: icon_aksi = "🔴"
            else: icon_aksi = "🟡"

            pesan_utama = f"🚨 *BTC-IDR MARKET ALERT* 🚨\n"
            ...
            pesan_utama += f"💡 {alasan_saran}\n\n"
            
            pesan_utama += f"💰 *HARGA SAAT INI:* {format_rupiah(indodax_live_idr)}\n"
            pesan_utama += f"📈 *PERGERAKAN:* {teks_perubahan}\n"
            pesan_utama += f"🌊 *KONDISI PASAR:* {current_regime} | F&G: {fng_str}\n\n"
            
            pesan_utama += f"🤖 *PREDIKSI AI (FEE ADJUSTED):*\n"
            pesan_utama += f"• 1 Jam Kedepan: *{arah_1h}* ({prob_1h*100:.1f}%)\n"
            pesan_utama += f"• 6 Jam Kedepan: *{arah_6h}* ({prob_6h*100:.1f}%)\n\n"
            
            pesan_utama += f"⚖️ *ORDERBOOK:* {status_orderbook}\n"
            pesan_utama += f"🛡️ *REKOMENDASI MODAL:* Gunakan *{exposure}%* portfolio\n\n"
            
            pesan_utama += f"✅ _Akurasi History AI (1H): {eval_1h}_\n"
            
            if risk_mult < 1.0:
                pesan_utama += f"⚠️ _Sistem Otomatis Menurunkan Risiko Modal (Akurasi History Berubah)_\n"

            pesan_diag = ""
            if ada_error_sistem:
                pesan_diag += f"*SYSTEM WARNING:*\n"
