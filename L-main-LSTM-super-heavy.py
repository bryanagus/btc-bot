# ==============================================================================
# BTC QUANT GODMODE PRO MAX - CYBORG LAPTOP EDITION (SUPER HEAVY)
# AI Core Upgrade: XGBoost + LightGBM + TensorFlow LSTM (Long Short-Term Memory)
# Fitur: Fast Backoff, Auto-Risk, Monte Carlo, Kelly Sizing, 3 Full Charts
# Laporan: Radar Roket (Overbought), Radar Pisau Jatuh (Oversold), Smart Diagnostic
# Evaluasi Murni: USD Global (Yahoo Finance) 
# Engine: Infinite Loop (1 Menit Interval) dengan Cache AI 3 Jam.
# ==============================================================================
import os
# Menyembunyikan log merah peringatan bawaan TensorFlow agar terminal rapi
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

import pandas as pd
import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
import pytz
import requests
import warnings
import xml.etree.ElementTree as ET
import time
import csv
import json
import pickle
import logging
from datetime import datetime

# ================= IMPORT KECERDASAN TINGKAT DEWA =================
import yfinance as yf
from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer
from sklearn.preprocessing import StandardScaler
from sklearn.calibration import CalibratedClassifierCV
from sklearn.model_selection import TimeSeriesSplit

# 1. XGBoost (Sang Raja Kompetisi Trading)
import xgboost as xgb
# 2. LightGBM (Mesin Cerdas Anti-Outlier)
import lightgbm as lgb
# 3. TensorFlow LSTM (Mata Sakti Pelihat Masa Depan 3D)
from tensorflow.keras.models import Sequential, load_model
from tensorflow.keras.layers import LSTM, Dense, Dropout, Input
from tensorflow.keras.callbacks import EarlyStopping

warnings.filterwarnings('ignore')
pd.options.mode.chained_assignment = None 

# ================= KONFIGURASI =================
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN') 
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')
HISTORY_FILE = "ai_history_log.csv" 
WEB_DATA_FILE = "dashboard_data.json"
ALERT_FILE = "last_alert_state.json"
# ===============================================

# ================= SISTEM LOGGING PROFESIONAL =================
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("bot_system.log"),
        logging.StreamHandler()
    ]
)

diagnostics = {
    "api": "✅ Normal (API Terhubung)",
    "csv": "✅ Sinkron (Hybrid Format Terbaca)",
    "ai_1h": "✅ Optimal (XGB+LGBM+LSTM)",
    "ai_6h": "✅ Optimal (XGB+LGBM+LSTM)",
    "chart": "✅ Optimal (3 Grafik PNG)",
    "web3": "✅ JSON Hybrid Terupdate"
}

def format_rupiah(angka):
    if pd.isna(angka): return "Rp 0"
    return f"Rp {angka:,.0f}".replace(',', '.')

def format_usd(angka):
    if pd.isna(angka): return "$0"
    return f"${angka:,.2f}"

# ------------------------------------------------------------------------------
# 1. MODUL FETCH DATA & NEWS (HYBRID, UTC ENGINE)
# ------------------------------------------------------------------------------
def fetch_fear_and_greed():
    try:
        resp = requests.get("https://api.alternative.me/fng/?limit=100", timeout=10).json()
        fng_dict = {pd.to_datetime(x['timestamp'], unit='s', utc=True).date(): float(x['value']) for x in resp['data']}
        return fng_dict
    except Exception as e:
        diagnostics["api"] = diagnostics["api"].replace("✅ Normal", "🟡 Peringatan") + " | 🟡 F&G API Gagal"
        return {}

def fetch_data_with_retry(period='730d', interval='1h'):
    delays = [5, 15, 30]
    for attempt, delay in enumerate(delays + [0]):
        try:
            df = yf.download('BTC-USD', period=period, interval=interval, progress=False)
            if isinstance(df.columns, pd.MultiIndex): 
                df.columns = df.columns.droplevel(1)
            
            df.index = pd.to_datetime(df.index, utc=True)
            
            idr_data = yf.download('IDR=X', period='5d', progress=False)
            if isinstance(idr_data.columns, pd.MultiIndex): 
                idr_data.columns = idr_data.columns.droplevel(1)
            kurs_idr = float(idr_data['Close'].dropna().iloc[-1])
            
            try:
                indodax_req = requests.get('https://indodax.com/api/ticker/btcidr', timeout=10).json()
                indodax_live_idr = float(indodax_req['ticker']['last'])
            except Exception as e:
                indodax_live_idr = float(df['Close'].iloc[-1]) * kurs_idr
                diagnostics["api"] = "🟡 Ticker Indodax Down (Menggunakan Estimasi Kurs)"
            
            return df, kurs_idr, indodax_live_idr
            
        except Exception as e:
            if attempt == len(delays):
                diagnostics["api"] = "❌ Gagal Terhubung (Server API Global Down)"
                raise Exception(f"API Timeout: {e}")
            time.sleep(delay)

def fetch_indodax_depth():
    try:
        resp = requests.get('https://indodax.com/api/depth/btcidr', timeout=10).json()
        limit_buy = min(50, len(resp.get('buy', [])))
        limit_sell = min(50, len(resp.get('sell', [])))
        
        buy_wall = sum([float(x[0]) * float(x[1]) for x in resp.get('buy', [])[:limit_buy]])
        sell_wall = sum([float(x[0]) * float(x[1]) for x in resp.get('sell', [])[:limit_sell]])
        return buy_wall, sell_wall
    except Exception as e:
        return 0, 0

# ------------------------------------------------------------------------------
# 2. FEATURE ENGINEERING (ADVANCED QUANT INDICATORS)
# ------------------------------------------------------------------------------
def engineer_features(df, fng_dict):
    df = df.copy() 
    
    df['MA_50'] = df['Close'].rolling(window=50).mean()
    df['MA_200'] = df['Close'].rolling(window=200).mean()
    df['BB_Middle'] = df['Close'].rolling(window=20).mean()
    std_20 = df['Close'].rolling(window=20).std()
    df['BB_Upper'] = df['BB_Middle'] + (std_20 * 2.0)
    df['BB_Lower'] = df['BB_Middle'] - (std_20 * 2.0)
    
    delta = df['Close'].diff()
    up, down = delta.clip(lower=0), -1 * delta.clip(upper=0)
    rs = up.ewm(com=13, adjust=False).mean() / down.ewm(com=13, adjust=False).mean()
    df['RSI'] = 100 - (100 / (1 + rs))

    high_low = df['High'] - df['Low']
    high_close = np.abs(df['High'] - df['Close'].shift())
    low_close = np.abs(df['Low'] - df['Close'].shift())
    df['ATR'] = np.max(pd.concat([high_low, high_close, low_close], axis=1), axis=1).rolling(14).mean()
    
    df['+DM'] = np.where((df['High'] - df['High'].shift(1)) > (df['Low'].shift(1) - df['Low']), np.maximum(df['High'] - df['High'].shift(1), 0), 0)
    df['-DM'] = np.where((df['Low'].shift(1) - df['Low']) > (df['High'] - df['High'].shift(1)), np.maximum(df['Low'].shift(1) - df['Low'], 0), 0)
    df['+DI'] = 100 * (df['+DM'].ewm(alpha=1/14, adjust=False).mean() / df['ATR'])
    df['-DI'] = 100 * (df['-DM'].ewm(alpha=1/14, adjust=False).mean() / df['ATR'])
    df['ADX'] = (100 * np.abs(df['+DI'] - df['-DI']) / (df['+DI'] + df['-DI'])).ewm(alpha=1/14, adjust=False).mean()

    if 'Volume' in df.columns:
        df['VWAP_24'] = (((df['High'] + df['Low'] + df['Close']) / 3) * df['Volume']).rolling(24).sum() / df['Volume'].rolling(24).sum()
        df['Volume_Spike'] = df['Volume'] / df['Volume'].rolling(24).mean().replace(0, 1)
    else:
        df['VWAP_24'] = df['Close']
        df['Volume_Spike'] = 1.0

    df["EMA20"] = df["Close"].ewm(span=20).mean()
    df["EMA50"] = df["Close"].ewm(span=50).mean()
    df["EMA_Spread"] = (df["EMA20"] - df["EMA50"]) / df["Close"]
    
    ema12 = df["Close"].ewm(span=12).mean()
    ema26 = df["Close"].ewm(span=26).mean()
    df["MACD_Line"] = ema12 - ema26
    df["MACD_Signal"] = df["MACD_Line"].ewm(span=9).mean()
    df["MACD_Hist"] = df["MACD_Line"] - df["MACD_Signal"]
    
    df["Return_1H"] = df["Close"].pct_change()
    df["Volatility"] = df["Return_1H"].rolling(24).std()
    df["Trend_Slope"] = df["Close"].rolling(12).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x)==12 else 0, raw=True)
    df["Momentum_Accel"] = df["Return_1H"].diff()
    df["Regime"] = np.where(df["EMA20"] > df["EMA50"], 1, 0)
    
    df["Log_Return"] = np.log(df["Close"] / df["Close"].shift(1))
    df["BB_Width"] = (df["BB_Upper"] - df["BB_Lower"]) / df["BB_Middle"]
    df["ATR_Ratio"] = df["ATR"] / df["Close"]
    
    df['Date_Only'] = df.index.date
    df['FnG_Index'] = df['Date_Only'].map(fng_dict)
    df['FnG_Index'] = df['FnG_Index'].ffill().bfill().fillna(50.0) 
    df.drop(columns=['Date_Only'], inplace=True)

    df["Target_1H"] = (df["Close"].shift(-1) > df["Close"]).astype(float)
    df["Target_6H"] = (df["Close"].shift(-6) > df["Close"]).astype(float)
    
    features_cols = [
        "EMA_Spread", "RSI", "MACD_Hist", "Log_Return", "Volatility", 
        "Trend_Slope", "Momentum_Accel", "Regime", "ADX", "BB_Width", "ATR_Ratio", "Volume_Spike"
    ]
    return df, features_cols

# ------------------------------------------------------------------------------
# 3. MODUL AI SUPER HEAVY (XGBOOST, LIGHTGBM, TENSORFLOW LSTM)
# ------------------------------------------------------------------------------
def train_honest_model(df, features, target_col, shift_len, model_name):
    # Nama file dipisahkan untuk Scikit/XGB dan Keras/LSTM
    base_model_file = f"ai_base_{model_name}.pkl"
    lstm_model_file = f"ai_lstm_{model_name}.keras"
    TIMESTEPS = 12 # LSTM akan melihat sejarah 12 Jam ke belakang secara berurutan
    
    need_training = True
    
    # Cek Cache 3 Jam
    if os.path.exists(base_model_file) and os.path.exists(lstm_model_file):
        file_age = time.time() - os.path.getmtime(base_model_file)
        if file_age < 10800:
            need_training = False

    if not need_training:
        try:
            with open(base_model_file, 'rb') as f:
                models = pickle.load(f)
            xgb_cal = models['xgb']
            lgb_cal = models['lgb']
            scaler = models['scaler']
            lstm_model = load_model(lstm_model_file)
        except Exception as e:
            need_training = True

    if need_training:
        logging.info(f"[*] Melatih AI SUPER HEAVY ({model_name}) dengan XGBoost, LightGBM, & TensorFlow LSTM...")
        
        train_df = df.iloc[:-shift_len].dropna(subset=features + [target_col])
        X_train_raw = train_df[features].values
        y_train_raw = train_df[target_col].values
        
        # Scaling Data
        scaler = StandardScaler()
        X_train_scaled = scaler.fit_transform(X_train_raw)

        # ==========================================================
        # PERSIAPAN DATA 3D UNTUK TENSORFLOW LSTM
        # ==========================================================
        X_train_3d, y_train_3d = [], []
        for i in range(len(X_train_scaled) - TIMESTEPS):
            X_train_3d.append(X_train_scaled[i : i + TIMESTEPS])
            y_train_3d.append(y_train_raw[i + TIMESTEPS])
        X_train_3d = np.array(X_train_3d)
        y_train_3d = np.array(y_train_3d)

        # Cross Validation untuk XGBoost dan LightGBM
        tscv = TimeSeriesSplit(n_splits=5)
        
        # --- MODEL 1: XGBoost ---
        xgb_base = xgb.XGBClassifier(n_estimators=150, max_depth=6, learning_rate=0.05, eval_metric='logloss', random_state=42, n_jobs=-1)
        xgb_cal = CalibratedClassifierCV(xgb_base, method="sigmoid", cv=tscv)
        xgb_cal.fit(X_train_scaled, y_train_raw)
        
        # --- MODEL 2: LightGBM ---
        lgb_base = lgb.LGBMClassifier(n_estimators=150, max_depth=6, learning_rate=0.05, random_state=42, n_jobs=-1, verbose=-1)
        lgb_cal = CalibratedClassifierCV(lgb_base, method="sigmoid", cv=tscv)
        lgb_cal.fit(X_train_scaled, y_train_raw)
        
        # --- MODEL 3: TENSORFLOW LSTM (DEEP LEARNING TIME SERIES) ---
        logging.info(f"[*] Memulai training TensorFlow LSTM ({model_name}). Harap bersabar...")
        lstm_model = Sequential([
            Input(shape=(TIMESTEPS, len(features))),
            LSTM(64, return_sequences=True),
            Dropout(0.2),
            LSTM(32),
            Dropout(0.2),
            Dense(1, activation='sigmoid')
        ])
        lstm_model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
        # EarlyStopping agar tidak overfitting dan training berhenti jika sudah optimal
        es = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
        # Training dengan validasi 15% data terakhir
        lstm_model.fit(X_train_3d, y_train_3d, epochs=30, batch_size=64, validation_split=0.15, verbose=0, callbacks=[es])
        
        # Save Model
        with open(base_model_file, 'wb') as f:
            pickle.dump({'xgb': xgb_cal, 'lgb': lgb_cal, 'scaler': scaler}, f)
        lstm_model.save(lstm_model_file)
        logging.info(f"[*] Training ({model_name}) selesai!")

    # ==========================================================
    # MEMPREDIKSI HARGA SAAT INI (REAL-TIME)
    # ==========================================================
    # 1. Prediksi 2D untuk XGBoost dan LightGBM
    X_live_2d = df[features].iloc[-1:].fillna(0).values
    X_live_2d_scaled = scaler.transform(X_live_2d)
    
    prob_xgb = xgb_cal.predict_proba(X_live_2d_scaled)[0,1]
    prob_lgb = lgb_cal.predict_proba(X_live_2d_scaled)[0,1]

    # 2. Prediksi 3D untuk TensorFlow LSTM
    # Mengambil 12 Jam terakhir (TIMESTEPS)
    X_live_seq = df[features].iloc[-TIMESTEPS:].fillna(0).values
    X_live_seq_scaled = scaler.transform(X_live_seq)
    # Reshape menjadi (1 sample, 12 timesteps, features)
    X_live_3d = np.array([X_live_seq_scaled])
    
    # LSTM Predict mengembalikan array 2D [[prob]]
    prob_lstm = lstm_model.predict(X_live_3d, verbose=0)[0][0]
    
    # PEMBOBOTAN DEWA: 35% XGBoost, 35% LightGBM, 30% LSTM (Memori Jangka Panjang)
    prob_final = (prob_xgb * 0.35) + (prob_lgb * 0.35) + (prob_lstm * 0.30)
    return prob_final

# ------------------------------------------------------------------------------
# 4. DATABASE HYBRID & HISTORY EVALUATION
# ------------------------------------------------------------------------------
def get_historical_indodax_price(t_target_utc, past_usd, past_idr, actual_usd):
    try:
        t_unix = int(t_target_utc.timestamp())
        url = f"https://indodax.com/tradingview/history?symbol=BTCIDR&resolution=60&from={t_unix-3600}&to={t_unix+3600}"
        resp = requests.get(url, timeout=10).json()
        if resp.get('s') == 'ok':
            times = resp.get('t', [])
            closes = resp.get('c', [])
            for i, ts in enumerate(times):
                if abs(ts - t_unix) <= 120:
                    return float(closes[i])
    except:
        pass
    return actual_usd * (past_idr / past_usd)

def manage_history_and_evaluate(df, prob_1h, prob_6h, indodax_live_idr, kurs_idr):
    waktu_eksekusi = datetime.now(pytz.utc)
    candle_time = df.index[-1] 
    
    usd_live_price = float(df['Close'].iloc[-1])
    target_time_1h = candle_time + pd.Timedelta(hours=1)
    target_time_6h = candle_time + pd.Timedelta(hours=6)
    
    eval_1h_msg = "Menunggu..."
    eval_6h_msg = "Menunggu..."
    risk_multiplier = 1.0 
    
    file_exists = os.path.isfile(HISTORY_FILE)
    
    try:
        if file_exists:
            history = pd.read_csv(HISTORY_FILE)
            history['target_1h'] = pd.to_datetime(history['target_1h'], format='mixed', utc=True)
            history['target_6h'] = pd.to_datetime(history['target_6h'], format='mixed', utc=True)
            
            mask_1h = (history['target_1h'] <= candle_time) & (history['result_1h'].isna())
            for idx, row in history[mask_1h].iterrows():
                t_target = row['target_1h']
                t_target_candle = t_target - pd.Timedelta(hours=1)
                
                if t_target_candle in df.index:
                    past_prob = float(row['prob_1h'])
                    past_usd_price = float(row['usd_start_price'])
                    past_idr_price = float(row['idr_start_price'])
                    actual_end_price = float(df.loc[t_target_candle, 'Close'])
                    
                    if actual_end_price > past_usd_price * 1.002: arah_asli = "Naik Signifikan 🚀"
                    elif actual_end_price > past_usd_price: arah_asli = "Naik Dikit 📈"
                    elif actual_end_price < past_usd_price * 0.998: arah_asli = "Turun Signifikan 🩸"
                    else: arah_asli = "Turun Dikit 📉"
                        
                    if (past_prob > 0.5 and actual_end_price > past_usd_price) or (past_prob <= 0.5 and actual_end_price <= past_usd_price):
                        hasil = f"BENAR ✅ (Realita: {arah_asli})"
                    else: hasil = f"SALAH ❌ (Realita: {arah_asli})"
                        
                    history.loc[idx, 'result_1h'] = hasil
                    history.loc[idx, 'usd_end_price_1h'] = actual_end_price
                    history.loc[idx, 'idr_end_price_1h'] = indodax_live_idr if t_target == candle_time else get_historical_indodax_price(t_target, past_usd_price, past_idr_price, actual_end_price)
                    if t_target == candle_time: eval_1h_msg = hasil

            mask_6h = (history['target_6h'] <= candle_time) & (history['result_6h'].isna())
            for idx, row in history[mask_6h].iterrows():
                t_target = row['target_6h']
                t_target_candle = t_target - pd.Timedelta(hours=1)
                
                if t_target_candle in df.index:
                    past_prob = float(row['prob_6h'])
                    past_usd_price = float(row['usd_start_price'])
                    past_idr_price = float(row['idr_start_price'])
                    actual_end_price = float(df.loc[t_target_candle, 'Close'])
                    
                    if actual_end_price > past_usd_price * 1.006: arah_asli = "Naik Signifikan 🚀"
                    elif actual_end_price > past_usd_price: arah_asli = "Naik Dikit 📈"
                    elif actual_end_price < past_usd_price * 0.994: arah_asli = "Turun Signifikan 🩸"
                    else: arah_asli = "Turun Dikit 📉"
                        
                    if (past_prob > 0.5 and actual_end_price > past_usd_price) or (past_prob <= 0.5 and actual_end_price <= past_usd_price):
                        hasil = f"BENAR ✅ (Realita: {arah_asli})"
                    else: hasil = f"SALAH ❌ (Realita: {arah_asli})"
                        
                    history.loc[idx, 'result_6h'] = hasil
                    history.loc[idx, 'usd_end_price_6h'] = actual_end_price
                    history.loc[idx, 'idr_end_price_6h'] = indodax_live_idr if t_target == candle_time else get_historical_indodax_price(t_target, past_usd_price, past_idr_price, actual_end_price)
                    if t_target == candle_time: eval_6h_msg = hasil

            history.to_csv(HISTORY_FILE, index=False)

            recent_1h = history.dropna(subset=['result_1h']).tail(24)
            if len(recent_1h) >= 3:
                benar_count = recent_1h['result_1h'].str.contains('BENAR').sum()
                win_rate = benar_count / len(recent_1h)
                if win_rate < 0.4: risk_multiplier = 0.1     
                elif win_rate < 0.6: risk_multiplier = 0.5   
                else: risk_multiplier = 1.0                  
                
    except Exception as e:
        risk_multiplier = 1.0

    try:
        t_curr_str = waktu_eksekusi.strftime('%Y-%m-%dT%H:%M:%SZ')
        t_1h_str = target_time_1h.strftime('%Y-%m-%dT%H:%M:%SZ')
        t_6h_str = target_time_6h.strftime('%Y-%m-%dT%H:%M:%SZ')
        
        is_duplicate = False
        if file_exists:
            try:
                # Cegah duplikat dalam 15 menit agar CSV tidak terlalu gemuk di laptop
                last_time_str = str(history['created_at'].iloc[-1])
                last_time_obj = datetime.strptime(last_time_str[:19], '%Y-%m-%dT%H:%M:%S')
                if (waktu_eksekusi.replace(tzinfo=None) - last_time_obj).total_seconds() < 800:
                    is_duplicate = True
            except: pass

        if not is_duplicate:
            with open(HISTORY_FILE, mode='a', newline='') as file:
                writer = csv.writer(file)
                if not file_exists:
                    writer.writerow(['created_at', 'kurs_usd_idr', 'usd_start_price', 'idr_start_price', 'target_1h', 'prob_1h', 'usd_end_price_1h', 'idr_end_price_1h', 'result_1h', 'target_6h', 'prob_6h', 'usd_end_price_6h', 'idr_end_price_6h', 'result_6h'])
                writer.writerow([t_curr_str, kurs_idr, usd_live_price, indodax_live_idr, t_1h_str, prob_1h, '', '', '', t_6h_str, prob_6h, '', '', ''])
    except: pass

    return eval_1h_msg, eval_6h_msg, risk_multiplier

def generate_web3_dashboard_data(indodax_idr, global_usd, kurs_idr, prob_1h, prob_6h, df, risk_mult):
    global_idr_converted = global_usd * kurs_idr
    spread_premium = indodax_idr - global_idr_converted
    try:
        if not os.path.isfile(HISTORY_FILE): return
        history = pd.read_csv(HISTORY_FILE)
        
        hist_1h = history.dropna(subset=['result_1h']).tail(100)
        table_1h = [{"waktu": pd.to_datetime(row['created_at'], format='mixed', utc=True).tz_convert('Asia/Makassar').strftime('%d %b %H:%M'), "start_usd": row['usd_start_price'], "end_usd": row['usd_end_price_1h'], "prediksi": "NAIK" if row['prob_1h'] > 0.5 else "TURUN", "status": row['result_1h'].split(' ')[0]} for _, row in hist_1h.iterrows()]
        
        total_1h = len([x for x in table_1h if "BENAR" in x['status']])

        web_data = {
            "last_update": datetime.now(pytz.timezone('Asia/Makassar')).strftime('%d %B %Y %H:%M WITA'),
            "prices": {"indodax_idr": indodax_idr, "global_usd": global_usd, "spread_premium": spread_premium},
            "current_prediction": {"prob_1h": prob_1h, "prob_6h": prob_6h},
            "stats": {"win_rate_1h": round((total_1h / len(table_1h) * 100) if table_1h else 0, 1), "risk_multiplier": risk_mult},
        }

        with open(WEB_DATA_FILE, 'w') as f: json.dump(web_data, f, indent=4)
    except: pass

# ------------------------------------------------------------------------------
# 5. RISK MANAGEMENT & VISUALISASI
# ------------------------------------------------------------------------------
def monte_carlo_simulation_log(price, log_returns_series, steps=1, sims=2000):
    historical_returns = log_returns_series.dropna().values
    paths = []
    for _ in range(sims):
        sampled_returns = np.random.choice(historical_returns, size=steps)
        prices = [price]
        for r in sampled_returns: prices.append(prices[-1] * np.exp(r))
        paths.append(prices)
    final_prices = np.array(paths)[:, -1]
    return np.mean(final_prices), np.percentile(final_prices, 5)

def position_sizing_kelly(prob_1h, prob_6h, atr, risk_multiplier):
    avg_prob = (prob_1h + prob_6h) / 2
    if avg_prob < 0.5: return 0.0
    reward, risk = atr * 3.5, atr * 1.5
    if risk <= 0: return 0.0 
    edge = avg_prob - ((1 - avg_prob) / (reward/risk))
    size = min(max(edge, 0), 0.25) * risk_multiplier 
    return round(size * 100, 2)

def plot_professional_analysis(df, prob_1h, prob_6h, atr, base_filename="chart_main"):
    plt.clf() 
    plot_data = df.tail(80).copy() 
    plot_data.index = plot_data.index.tz_convert('Asia/Makassar')
    plt.style.use('seaborn-v0_8-darkgrid')
    fig = plt.figure(figsize=(14, 8))
    plt.plot(plot_data.index, plot_data['Close'], label='Harga Global (USD)', color='black', linewidth=2, zorder=5)
    plt.plot(plot_data.index, plot_data['VWAP_24'], label='VWAP', color='#ff7f0e', linestyle='-.')
    last_time, last_price = plot_data.index[-1], plot_data['Close'].iloc[-1]
    t_1h, p_1h = last_time + pd.Timedelta(hours=1), last_price + (atr * (prob_1h - 0.5) * 2)
    plt.scatter(t_1h, p_1h, color='blue', s=80, label='Target 1 Jam (XGB+LSTM)')
    plt.plot([last_time, t_1h], [last_price, p_1h], color='blue', linestyle='--')
    plt.title('GLOBAL MARKET - SUPER HEAVY CYBORG LSTM EDITION', fontsize=16, fontweight='bold')
    plt.legend(loc='upper left')
    plt.tight_layout()
    plt.savefig(f"{base_filename}.png", dpi=100)
    plt.close('all')

def plot_zoomed_analysis(df, prob_1h, prob_6h, atr, base_filename="chart_zoom"):
    plt.clf()
    plot_data = df.tail(12).copy()
    plot_data.index = plot_data.index.tz_convert('Asia/Makassar')
    plt.style.use('seaborn-v0_8-darkgrid')
    fig, ax1 = plt.subplots(figsize=(12, 6))
    ax1.plot(plot_data.index, plot_data['Close'], color='black', linewidth=3)
    ax1.fill_between(plot_data.index, plot_data['BB_Upper'], plot_data['BB_Lower'], color='gray', alpha=0.15)
    ax1.set_title('🔍 ZOOM BOLLINGER', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"{base_filename}.png", dpi=100)
    plt.close('all')

def plot_dashboard_indicators(df, base_filename="chart_indicators"):
    plt.clf()
    plot_data = df.tail(80).copy()
    plot_data.index = plot_data.index.tz_convert('Asia/Makassar')
    plt.style.use('seaborn-v0_8-darkgrid')
    fig = plt.figure(figsize=(14, 8))
    gs = fig.add_gridspec(2, 1, height_ratios=[1, 1], hspace=0.3)
    
    ax2 = fig.add_subplot(gs[0, 0])
    ax2.plot(plot_data.index, plot_data['RSI'], color='purple', linewidth=2)
    ax2.axhline(70, color='red', linestyle='--', label='Overbought (Jual)')
    ax2.axhline(30, color='green', linestyle='--', label='Oversold (Beli)')
    ax2.set_title('RSI (Momentum Cepat)')
    ax2.legend()

    ax3 = fig.add_subplot(gs[1, 0])
    ax3.bar(plot_data.index, plot_data['MACD_Hist'], color=np.where(plot_data['MACD_Hist']>0, 'green', 'red'))
    ax3.set_title('MACD Histogram')
    plt.tight_layout()
    plt.savefig(f"{base_filename}.png", dpi=100)
    plt.close('all')

# ------------------------------------------------------------------------------
# 6. TELEGRAM SENDER
# ------------------------------------------------------------------------------
def send_telegram_messages(pesan_utama, chart_paths, pesan_diag="", pesan_saran=""):
    if not TELEGRAM_BOT_TOKEN: 
        print("MOHON SETTING TOKEN TELEGRAM DI TERMINAL ANDA DULU!")
        return
    url_msg = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    url_photo = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto"
    
    requests.post(url_msg, data={'chat_id': TELEGRAM_CHAT_ID, 'text': pesan_utama, 'parse_mode': 'Markdown'})
    for path in chart_paths:
        if os.path.exists(path):
            with open(path, 'rb') as photo:
                requests.post(url_photo, data={'chat_id': TELEGRAM_CHAT_ID}, files={'photo': photo})
    if pesan_saran.strip() != "":
        requests.post(url_msg, data={'chat_id': TELEGRAM_CHAT_ID, 'text': pesan_saran, 'parse_mode': 'Markdown'})

# ------------------------------------------------------------------------------
# MAIN EXECUTION (VERSI LOOPING LAPTOP 1 MENIT)
# ------------------------------------------------------------------------------
def run_bot_cycle():
    start_time = time.time()
    sekarang_wita = datetime.now(pytz.timezone('Asia/Makassar'))
    logging.info(f"=== Memulai Siklus Pantau Jam {sekarang_wita.strftime('%H:%M:%S WITA')} ===")
    
    try:
        fng_dict = fetch_fear_and_greed()
        df, kurs_idr, indodax_live_idr = fetch_data_with_retry()
        df, features = engineer_features(df, fng_dict)
        
        df_closed = df.iloc[:-1].copy()
        buy_wall, sell_wall = fetch_indodax_depth()
        
        global_usd = float(df['Close'].iloc[-1])
        global_idr_converted = global_usd * kurs_idr
        spread_premium = indodax_live_idr - global_idr_converted
        current_atr = df["ATR"].iloc[-1]
        
        # XGBoost & TensorFlow LSTM Training Block
        try: prob_1h = train_honest_model(df_closed, features, "Target_1H", shift_len=1, model_name="1H")
        except Exception as e: 
            logging.error(f"Error AI 1H: {e}"); prob_1h = 0.5
            
        try: prob_6h = train_honest_model(df_closed, features, "Target_6H", shift_len=6, model_name="6H")
        except Exception as e: 
            logging.error(f"Error AI 6H: {e}"); prob_6h = 0.5

        eval_1h, eval_6h, risk_mult = manage_history_and_evaluate(df, prob_1h, prob_6h, indodax_live_idr, kurs_idr)
        generate_web3_dashboard_data(indodax_live_idr, global_usd, kurs_idr, prob_1h, prob_6h, df, risk_mult)
        
        exposure = position_sizing_kelly(prob_1h, prob_6h, current_atr, risk_mult)
        
        def get_arah(prob):
            if prob >= 0.6: return "NAIK KUAT 🚀"
            elif prob > 0.5: return "Cenderung NAIK 📈"
            elif prob <= 0.4: return "TURUN KUAT 🚨"
            else: return "Cenderung TURUN 📉"
            
        arah_1h, arah_6h = get_arah(prob_1h), get_arah(prob_6h)
        fng_val = df['FnG_Index'].iloc[-1]
        rsi_sekarang = df['RSI'].iloc[-1]
        saran_tindakan, alasan_saran = "", ""
        
        # LOGIKA REKOMENDASI ANTI FOMO & ANTI GREED
        if prob_1h > 0.5 and prob_6h > 0.5 and spread_premium <= 0 and fng_val < 75:
            if rsi_sekarang < 35:
                saran_tindakan = "⚠️ TUNGGU DULU (PISAU JATUH)"
                alasan_saran = "Harga Indodax diskon, tapi Global MASIH MENUKIK (RSI Oversold). Jangan FOMO."
            else:
                saran_tindakan = "🟢 STRONG BUY (BELI SEKARANG)"
                alasan_saran = "AI LSTM memprediksi tren naik kuat, harga Indodax DISKON. Titik entry ideal."
        elif prob_1h > 0.5 and prob_6h > 0.5 and spread_premium > (global_idr_converted * 0.005):
            if rsi_sekarang > 70:
                saran_tindakan = "🤑 TAKE PROFIT MAX (JUAL SEKARANG)"
                alasan_saran = "Harga melambung tinggi (Overbought). Bandar bisa membanting kapan saja. Amankan untung!"
            else:
                saran_tindakan = "🟡 TAHAN / JUAL SEBAGIAN (TAKE PROFIT)"
                alasan_saran = "Tren naik, tapi harga Indodax SANGAT MAHAL (Premium tinggi). Jual sebagian."
        elif prob_1h <= 0.5 and prob_6h <= 0.5 and spread_premium > 0:
            saran_tindakan = "🔴 STRONG SELL (JUAL SEGERA)"
            alasan_saran = "Tren turun, tapi Indodax masih ditawar mahal. Manfaatkan untuk JUAL."
        elif prob_1h <= 0.5 and prob_6h <= 0.5 and spread_premium < 0:
            saran_tindakan = "⚪ WAIT AND SEE (JANGAN BELI DULU)"
            alasan_saran = "Harga Indodax diskon, tapi AI memprediksi harga MASIH AKAN TURUN. Tahan peluru."
        else:
            saran_tindakan = "⚪ NETRAL / TAHAN POSISI"
            alasan_saran = "Sinyal bertabrakan. AI sedang menghitung pola 3D baru."

        kirim_telegram, alasan_kirim = False, ""
        
        # LOGIKA ALARM (1 MENITAN CEREWET)
        if not os.path.exists(ALERT_FILE):
            kirim_telegram, alasan_kirim = True, "Sistem LSTM Dinyalakan"
        else:
            try:
                with open(ALERT_FILE, 'r') as f: last_state = json.load(f)
                last_price_saved = last_state.get('price', global_usd)
                raw_diff_pct = ((global_usd - last_price_saved) / last_price_saved) * 100 if last_price_saved > 0 else 0.0
                price_diff = abs(raw_diff_pct)
                selisih_waktu = time.time() - last_state.get('time', 0)
                
                # Alarm harga drastis untuk laptop (0.8%)
                if price_diff >= 0.8:
                    kirim_telegram, alasan_kirim = True, f"Pergerakan Ekstrem"
                elif saran_tindakan in ["🟢 STRONG BUY (BELI SEKARANG)", "🔴 STRONG SELL (JUAL SEGERA)", "⚠️ TUNGGU DULU (PISAU JATUH)", "🤑 TAKE PROFIT MAX (JUAL SEKARANG)"]:
                    if saran_tindakan != last_state.get('saran', ''):
                        kirim_telegram, alasan_kirim = True, f"🚨 SINYAL DARURAT LSTM!"
                elif saran_tindakan != last_state.get('saran', ''):
                    if selisih_waktu > 900: # 15 menit cooldown untuk sinyal receh
                        kirim_telegram, alasan_kirim = True, f"Perubahan Strategi"
            except: pass

        if kirim_telegram:
            plot_professional_analysis(df, prob_1h, prob_6h, current_atr, "chart_main")
            plot_zoomed_analysis(df, prob_1h, prob_6h, current_atr, "chart_zoom")
            plot_dashboard_indicators(df, "chart_indicators")
            
            with open(ALERT_FILE, 'w') as f:
                json.dump({'price': global_usd, 'saran': saran_tindakan, 'time': time.time()}, f)

            pesan_utama = f"💎 *BOT LAPTOP (TENSORFLOW LSTM)* 💎\n"
            pesan_utama += f"📅 {sekarang_wita.strftime('%H:%M:%S WITA')}\n"
            pesan_utama += f"🔔 *Pemicu:* {alasan_kirim}\n\n"
            pesan_utama += f"📊 *Global:* {format_usd(global_usd)}\n"
            pesan_utama += f"🇮🇩 *Indodax:* {format_rupiah(indodax_live_idr)}\n"
            if spread_premium > 0: pesan_utama += f"💵 *Selisih:* {format_rupiah(abs(spread_premium))} (Premium)\n\n"
            else: pesan_utama += f"💵 *Selisih:* {format_rupiah(abs(spread_premium))} (Diskon)\n\n"
            
            pesan_saran = f"🎯 *KESIMPULAN:*\n*{saran_tindakan}*\n_{alasan_saran}_"
            send_telegram_messages(pesan_utama, ["chart_main.png", "chart_zoom.png", "chart_indicators.png"], "", pesan_saran)
            logging.info(f"[*] Pesan Telegram terkirim: {alasan_kirim}")
        else:
            logging.info(f"[*] Harga stabil. Memantau... (Waktu: {time.time() - start_time:.1f} dtk)")

    except Exception as fatal_e:
        logging.critical(f"FATAL ERROR SIKLUS: {str(fatal_e)}")

def main():
    print("=========================================================")
    print("🚀 BOT CYBORG DIAKTIFKAN (XGBOOST + LIGHTGBM + LSTM) 🚀")
    print("Bot akan mengecek pasar setiap 60 Detik.")
    print("Tekan CTRL+C di keyboard untuk mematikan bot.")
    print("=========================================================")
    
    while True:
        try:
            run_bot_cycle()
            time.sleep(60) # Tidur 1 menit
        except KeyboardInterrupt:
            print("\n⛔ Bot dimatikan secara manual. Sampai jumpa!")
            break
        except Exception as e:
            time.sleep(60)

if __name__ == "__main__":
    main()
