...
def engineer_features(df, fng_dict):
    ...
    # 4️⃣ Market Structure Feature Engineering
    ...
    # 2️⃣ Regime-Specific Logic (Data Categorization)
    df["Regime_Str"] = "RANGE"
    df.loc[df["ATR_Ratio"] > 0.006, "Regime_Str"] = "TREND"
    df.loc[df["Volatility"] > 0.01, "Regime_Str"] = "VOLATILE"
    # Fitur Regime numerik tetap dipertahankan untuk kompabilitas model lama jika diperlukan
    ...
    # 1️⃣ Multi-Horizon Target Labeling
...
# 5️⃣ Walk-Forward Training & 2️⃣ Regime-Specific Models
def train_honest_model(df, features, target_col, shift_len, model_name, current_regime):
    ...
    if need_training:
        ...
        # Saring data historis HANYA untuk rezim yang sedang terjadi
        regime_df = train_df[train_df["Regime_Str"] == current_regime]
        
        # Fallback: Jika data rezim spesifik terlalu sedikit (<100 baris), gunakan seluruh data
...
# 3️⃣ Dynamic Probability Threshold Parameter
...
def main():
    ...
    try:
        ...
        # Pengecekan Current Regime dari baris paling terakhir
        current_regime = df["Regime_Str"].iloc[-1]
        
        # Hitung Dynamic Threshold berdasarkan volatilitas saat ini
        ...
        if kirim_telegram:
            ...
            pesan_utama += f"• Dynamic Threshold: {threshold_1h*100:.1f}%\n"
            ...
            pesan_utama += f"• Dynamic Threshold: {threshold_6h*100:.1f}%\n"
